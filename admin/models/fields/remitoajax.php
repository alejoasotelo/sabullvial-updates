 ...
 * @since  0.0.1
 */
class JFormFieldRemitoAjax extends JFormFieldList
{
    /**
     * The field type.
     *
     * @var         string
     */
    public $type = 'RemitoAjax';

    protected $url = 'index.php?option=com_sabullvial&view=remitos&format=json';
    ...
    public function ajaxField($selector)
    {
        ...
        HTMLHelper::script('com_sabullvial/fields/'.$fieldType.'.js', $tagOptions);
    ...
    protected function getOptions()
    {
        ...
        $value = $this->value;
        if (!empty($value)) {
            JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_sabullvial/models', 'SabullvialModel');
            $model = JModelLegacy::getInstance('Remitos', 'SabullvialModel', ['ignore_request' => true]);
            $model->setState('filter.search', 'id:'.$value);
            $items = $model->getItems();
            foreach ($items as $item) {
                $option = JHtml::_('select.option', $item->id, $item->id);
                $option->optionattr = [
                    'data-id' => $item->id,
