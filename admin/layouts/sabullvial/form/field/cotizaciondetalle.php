...
// Create the modal id.
$modalId = 'CotizacionDetalle_' . $field->id;
...
if ($allowSelect) {
    ...
    if (!isset($scriptSelect[$field->id])) {
        JFactory::getDocument()->addScriptDeclaration("
			function jSelectCotizacionDetalle_" . $field->id . "(id, codigo_sap, nombre, marca, precio, stock) {
				window.processModalCotizacionDetalleSelect('CotizacionDetalle', '" . $field->name . "', '" . $field->id . "', id, codigo_sap, nombre, marca, precio, stock);
			}

            /*jQuery(document).ready(function() {
                var subform = jQuery('input[name=\"jform[cotizaciondetalles]\"]').parent().find('div.subform-repeatable').subformRepeatable();
                subform.off('subform-row-add').on('subform-row-add', function(e, row) {
                    row.remove();
                    jQuery('#ModalSelect$modalId').modal('show');
                })
            });*/
		");
...
$urlSelect = $linkArticles . '&amp;function=jSelectCotizacionDetalle_' . $field->id;
...
<span class="input-append">
    ...
    <?php if ($allowClear) : ?>
        <button type="button" class="btn<?php echo($value ? '' : ' hidden'); ?>" id="<?php echo $field->id . '_clear'; ?>" onclick="window.processModalCotizacionDetalleParent('<?php echo $field->name; ?>', '<?php echo $field->id; ?>'); return false;">
    ...
    <thead>
        <tr>
            ...
            <th width="1%" class="nowrap">
                <?php echo JText::_('COM_SABULLVIAL_PRODUCTOS_CANTIDAD'); ?>
            </th>
            <th width="1%" class="nowrap hidden-phone">
                <?php echo JText::_('COM_SABULLVIAL_PRODUCTOS_STOCK'); ?>
            </th>
            <th width="1%" class="nowrap">
                <?php echo JText::_('COM_SABULLVIAL_PRODUCTOS_ELIMINAR'); ?>
            </th>
        </tr>
    </thead>
    <tbody>
            ...
            <?php foreach ($items as $i => $row) : ?>
                <tr>
                    ...
                    <td class="nowrap small hidden-phone">
                        <?php echo PriceHelper::format($row->precio); ?>
                    </td>
                    <td>
                        <input type="text" name="<?php echo str_replace($field->fieldname, $field->fieldname.'_cantidad', $field->name);?>" value="<?php echo $row->cantidad; ?>"/>
                    </td>
...
<input type="hidden" id="<?php echo $field->id . '_id'; ?>" <?php echo $class; ?> 
    data-required="<?php echo (int) $field->required; ?>" 
    data-text="<?php echo htmlspecialchars(JText::_('COM_SABULLVIAL_SELECCIONAR_PRODUCTO'), ENT_COMPAT, 'UTF-8'); ?>" 
    data-fieldname-cantidad="<?php echo str_replace($field->fieldname, $field->fieldname.'_cantidad', $field->name);?>"
    value="" />